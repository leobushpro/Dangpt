...
model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PPO_POLICY.pt')
...
class MyBot(Bot):
    ...
    def initialize(self):
        ...
        Obs = DefaultObs(zero_padding=3,
                           ...
                           boost_coef=1.0)  # rlgym_compat already stores boost as 0-1 (boost/100), unlike rlgym v2 which uses 0-100
        ...
        # Pass self.match_config as the second argument
        self.extra_info = SimExtraInfo(self.field_info, self.match_config, tick_skip=self.tick_skip)
    ...
    def get_output(self, packet: GamePacket) -> ControllerState:
        ...
        if self.ticks >= self.tick_skip - 1:
            ...
            parsed_actions = self.action_parser.parse_actions(actions={self.player_id: action_idx}, state=self.game_state, shared_info={"sus": "sus"}).get(self.player_id)
